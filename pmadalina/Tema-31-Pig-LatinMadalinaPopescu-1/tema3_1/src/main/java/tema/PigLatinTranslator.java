      ...
      if(verificaRegula1(cuvantCuLitereMici)){
        ...
        if (verificaRegula3(cuvantCuLitereMici)){
          ...
          switch(cuvantCuLitereMici.substring(0,3)){
            ...
            case "sch":
            case "squ":
              cuvantTradus = cuvantCuLitereMici.substring(3) + cuvantCuLitereMici.substrig(0,3) + "ay";
              switchApelat = true;
              break;
            default:break;}
...
public static boolean verificaRegula1(String cuvant){
  String cuvantCuliteMici = cuvant.toLowerCase();
  String primaLitera = cuvantCuliteMici.substring(0,1);
  String primeleDouaLitere = cuvantCuliteMici.substring(0,2);
...
return false;
  ...
  String cuvantCuLitereMici = cuvant.toLowerCase();
  String primeleDoualLitere = cuvantCuLitereMici.substring(0,2);
  ...
  String[] consoane = {"th","thr","ch","sch","rh","squ","qu"};
   ...
   for(int j = 0; j < consoane.length; j++) {
       String toCampare = consoane[j];
      if((!verificaRegula1(cuvantCuLitereMici)) && (primeleDouaLitere.equals(toCompare) || primeleTreiLitere.equals(toCampare))) {
    return true;
  }
}
return false;
