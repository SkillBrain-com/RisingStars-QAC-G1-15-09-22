...
public String translateIndividualWord(String cuvant){
  ...
  } else { if( verificaRegula1(cuvantCuLitereMici)) {cuvantTradus = cuvantCuLitereMici + "ay";}
          else{ if(verificaRegula3(cuvantCuLitereMici)){ boolean switchApelat = false;
...
public static boolean verificaRegula1(String cuvant){
  ...
  for (int j = 0; j < vocale.length; j++) {
    ...
    if (primaLitera.equals(toCompare) || primeleDouaLitere.equals(toCompare)) {return true;}
  }
  return false;
}
  public static boolean verificaRegula2(String cuvant){return !verificaRegula1(cuvant);}
